...
API params initParams(INPUT int argc, INPUT char **argv)
{
    ...
    p.min_face = find_float_arg(argc, argv, "--minface", 24.0);
...
API int keep_one(INPUT detect* dets, INPUT int n, INPUT image im)
{
    ...
    for (i = 0; i < n; i++ ){
        bbox a = dets[i].bx;
        int ax = (int)(a.x2 + a.x1)/2;
        int ay = (int)(a.y2 + a.y1)/2;
        float dist = pow(pow(ax-cx, 2) + pow(ay-cy, 2), 0.5);
        if (dist < mindist){
            mindist = dist;
            index = i;
        }
