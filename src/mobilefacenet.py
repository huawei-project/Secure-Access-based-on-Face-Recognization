...
class MobileFacenet(nn.Module):
    ...
    Attributes:
        ...
        -> linear1  =    conv1x1 -> bn
    """
    def __init__(self, facesize=(112, 96), bottleneck_setting=Mobilefacenet_bottleneck_setting, ckpt='../ckpt'):
    ...
    def _load(self, ckptdir):
        ...
        pretrain = torch.load(
            os.path.join(ckptdir, 'MobileFacenet.pkl'), map_location='cpu')['net']
        toload = self.state_dict()
        update = {k: v for k, v in pretrain.items() if k in toload.keys()}
